...
def test_runtime_and_store_shims_reexport_platform_implementations() -> None:
    ...
    legacy_store = importlib.import_module("pyicloud.adapters.store.file_session_store")
    platform_store = importlib.import_module("pyicloud.platform.storage.session_store")
    assert legacy_store.FileSessionStoreAdapter is platform_store.FileSessionStoreAdapter
