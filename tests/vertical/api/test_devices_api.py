...
async def test_devices_api_list_location_and_status(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as client:
        ...
        assert location.json()["latitude"] == pytest.approx(40.4168)
        assert location.json()["longitude"] == pytest.approx(-3.7038)

        status = await client.get("/v1/devices/device-iphone-1/status", headers=headers)
        assert status.status_code == 200
        assert status.json()["batteryStatus"] == "Charging"
...
async def test_devices_api_actions_and_not_found(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as client:
        ...
        assert play.json()["detail"] == "Sound command sent"
        ...
        assert message.json()["detail"] == "Message command sent"
        ...
        assert lost.json()["detail"] == "Lost mode command sent"
