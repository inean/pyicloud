...
@pytest.mark.asyncio
async def test_cli_contacts_commands(app, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    contacts = await runner.invoke(main, ["--api-url", "http://testserver", "contacts", "list"])
    assert contacts.exit_code == 0
    assert [item["displayName"] for item in json.loads(contacts.output)] == ["Inean User", "Family Member"]
