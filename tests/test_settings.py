...
def test_account_dump_secret(secret, email):
    ...
    assert account.password and cast(Secret, account.password).get_secret_value() == secret
...
def test_account_model(secret, email):
    ...
    assert account.password and cast(Secret, account.password).get_secret_value() == password
    ...
    assert cast(Secret, account.password).get_secret_value() == secret
...
def test_settings_event_system(username, new_username):
    ...
    assert cast(Secret, settings.account.password).get_secret_value() == "password"
