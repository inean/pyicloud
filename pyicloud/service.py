...
class PyiCloudService:
    ...
    def list_devices(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.devices.list_devices(username=self._username)

    def device_location(self, *, device_id: str) -> Mapping[str, Any]:
        return self._bundle.devices.location(username=self._username, device_id=device_id)

    def device_status(self, *, device_id: str) -> Mapping[str, Any]:
        return self._bundle.devices.status(username=self._username, device_id=device_id)

    def device_play_sound(self, *, device_id: str, subject: str) -> None:
        self._bundle.devices.play_sound(username=self._username, device_id=device_id, subject=subject)

    def device_message(self, *, device_id: str, subject: str, message: str, sounds: bool) -> None:
        self._bundle.devices.display_message(
    ...
    def device_lost_mode(self, *, device_id: str, number: str, text: str, newpasscode: str) -> None:
        self._bundle.devices.lost_mode(
            username=self._username,
            device_id=device_id,
            number=number,
            text=text,
    ...
    def account_devices(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.accounts.account_devices(username=self._username)

    def account_family(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.accounts.account_family(username=self._username)

    def account_storage(self) -> Mapping[str, Any]:
        return self._bundle.accounts.account_storage(username=self._username)

    def drive_tree(self, *, path: str = "/") -> Mapping[str, Any]:
        return self._bundle.drive.tree(username=self._username, path=path)

    def drive_file_metadata(self, *, path: str) -> Mapping[str, Any]:
        return self._bundle.drive.file_metadata(username=self._username, path=path)

    def drive_file_content(self, *, path: str) -> bytes:
        return self._bundle.drive.file_content(username=self._username, path=path)

    def drive_create_folder(self, *, parent_path: str, name: str) -> None:
        self._bundle.drive.create_folder(username=self._username, parent_path=parent_path, name=name)

    def drive_upload_file(self, *, parent_path: str, filename: str, content: bytes) -> None:
        self._bundle.drive.upload_file(
            username=self._username,
    ...
    def drive_rename_node(self, *, path: str, new_name: str) -> None:
        self._bundle.drive.rename_node(username=self._username, path=path, new_name=new_name)

    def drive_delete_node(self, *, path: str) -> None:
        self._bundle.drive.delete_node(username=self._username, path=path)

    def calendar_calendars(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.calendars.calendars(username=self._username)
    ...
    def calendar_events(
        ...
        *,
        ...
        return self._bundle.calendars.events(
            username=self._username,
            from_dt=from_dt,
            to_dt=to_dt,
        )
    ...
    def calendar_event_detail(self, *, calendar_guid: str, event_guid: str) -> Mapping[str, Any]:
        return self._bundle.calendars.event_detail(
            username=self._username,
    ...
    def contacts_all(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.contacts.all_contacts(username=self._username)

    def reminders_lists(self) -> Mapping[str, Sequence[Mapping[str, Any]]]:
        return self._bundle.reminders.reminder_lists(username=self._username)
    ...
    def reminders_create(
        ...
        *,
        ...
        return self._bundle.reminders.create_reminder(
            username=self._username,
            title=title,
            description=description,
            collection=collection,
    ...
    def photos_albums(self) -> Sequence[Mapping[str, Any]]:
        return self._bundle.photos.list_albums(username=self._username)
    ...
    def photos_assets(
        ...
        *,
        ...
        return self._bundle.photos.list_assets(
            username=self._username,
            album=album,
            limit=limit,
            offset=offset,
    ...
    def photo_asset_metadata(self, *, asset_id: str, album: str = "All Photos") -> Mapping[str, Any]:
        return self._bundle.photos.asset_metadata(
            username=self._username,
            asset_id=asset_id,
            album=album,
        )
    ...
    def photo_asset_content(self, *, asset_id: str, album: str = "All Photos", version: str = "original") -> bytes:
        return self._bundle.photos.asset_content(
            username=self._username,
            asset_id=asset_id,
            album=album,
            version=version,
    ...
    def ubiquity_tree(self, *, path: str = "/") -> Mapping[str, Any]:
        return self._bundle.ubiquity.ubiquity_tree(username=self._username, path=path)

    def ubiquity_file_metadata(self, *, path: str) -> Mapping[str, Any]:
        return self._bundle.ubiquity.ubiquity_file_metadata(username=self._username, path=path)

    def ubiquity_file_content(self, *, path: str) -> bytes:
        return self._bundle.ubiquity.ubiquity_file_content(username=self._username, path=path)
